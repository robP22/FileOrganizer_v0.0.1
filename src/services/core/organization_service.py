...
class OrganizationService:
    ...
    def handle_organize_request(self, event):
        ...
        # Publish organization start event instead of logging paths
        event_bus.publish("organization_started", {
            "strategy": strategy,
            "status": "Starting organization process",
    ...
    def _create_organizer(self, strategy: str):
        """Factory method for creating organizers - reduces coupling"""
        organizer_map = {
            "date": "organizers.date_organizer.DateOrganizer",
    ...
    def organize_files_by_strategy(self, source_dir: str, destination_dir: str, strategy: str):
        # Create organizer using dynamic loading (reduces coupling)
        organizer = self._create_organizer(strategy)

        try:
            files_to_organize = self.scan_directory(source_dir)
            
            # Create progress tracker with unique ID
            ...
            for i, file_path in enumerate(files_to_organize):
                ...
                try:
                    ...
                    event_bus.publish("file_processed", {
                        ...
                        "organizer": organizer.get_strategy_name(),
                        "result": result
                    })
