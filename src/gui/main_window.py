...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        # FIXME add an abstraction for duplicate buttons with similar functionality
    ...
    def start_organization(self):
        ...
        if self.source_path and self.destination_path:
            event_bus.publish("organize_requested",
                    { "source_path": self.source_path, "destination_path": self.destination_path })
    ...
    def start_unorganizing(self):
        ...
        if self.source_path and self.destination_path:
            event_bus.publish("unorganize_requested",
                    { "source_directory": self.source_path, "target_directory": self.destination_path })

    def on_unorganize_completed(self, event):
        """ Handle successful unorganize completion """
        # FIXME cannot close the application when the message box is open
        from PySide6.QtWidgets import QMessageBox
